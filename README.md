...
5. Run (⌘R). The window should show the product version from Go (`VERSION` / `core.Version`, currently `0.0.0`). Onboarding (identity + `*.provenance` + `users`) is Go-testable; there is no first-run UI yet. Swift will pass Application Support and a parent folder into the core in PR7.
