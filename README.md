...
5. Run (⌘R). The window should show the product version from Go (`VERSION` / `core.Version`, currently `0.0.0`). Project folders and install `identity.json` are Go-testable; there is no onboarding UI yet. Swift will pass the Application Support directory into the core in a later PR.
