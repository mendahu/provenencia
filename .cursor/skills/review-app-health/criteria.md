...
Catch LLM- and rush-driven UI debt against Frost layers (**design system / recipes / snowflakes** — [`docs/design-system-layers.md`](../../../docs/design-system-layers.md)). This dimension **samples and flags**; for a full classify → cousins → compose-down → small-PR plan on one type, hand off to [`evaluate-ui-component`](../evaluate-ui-component/SKILL.md). For adding new chrome, [`add-ui-component`](../add-ui-component/SKILL.md).
...
- Target: `DesignSystem/Components/<Name>/` (content-agnostic), `DesignSystem/Recipes/<Name>/` (≥2 call sites), snowflakes under `Features/<Name>/` or `DesignSystem/Snowflakes/<Name>/`; store/FFI in `Platform/`. Flag new files still landing in legacy `Components/Core|Forms|Research/` or as loose `.swift` at a layer root.
- Prefer **one floating-menu kit** (`PVContextMenu` / `PVSelect` / jump menu — [Spike 5](../../../docs/deployment-plan/archive/spike-5/README.md)). Omnibar and `PVComboBox` keep separate hosts by design.
- Dialogs: compose `.pvFormDialog` / `.pvConfirm`; don’t redraw scrim/footer chrome in a feature sheet.
