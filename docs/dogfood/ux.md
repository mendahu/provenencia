...
- **Also wanted:** Some automatic data entry from the Artifact itself (image / PDF). The text is already on the page. An LLM is the obvious extractor, but it is not obvious how far we can get without one — even simple field population from a text layer (regex, known date/name patterns, paste-from-selection) would cut the retyping. Image-only scans have no text until OCR. Related parking-lot: [`ideas/pdf-text-find.md`](../ideas/pdf-text-find.md) (Find + text-selection paste; S7-07 left the viewer as a page raster).
- **Pulled into Spike 8 (first slice):** fill **transcription** only via Vision — [`S8-D1`](../deployment-plan/spike-8/design/S8-D1-auto-transcribe.md) / [`S8-01`](../deployment-plan/spike-8/deployment-plan.md). Shared Citation, graph+composer rethink, and Foundation Models stay here.
