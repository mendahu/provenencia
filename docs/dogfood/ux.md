...
- **Pulled into Spike 8 (first slice):** fill **transcription** via Vision on **image** Artifacts only — [`S8-D1`](../deployment-plan/spike-8/design/S8-D1-auto-transcribe.md) / [`S8-01`](../deployment-plan/spike-8/deployment-plan.md). PDF is descoped (paste / later text-layer, not OCR). Shared Citation, graph+composer rethink, and Foundation Models stay here.
