...
| 23 | **Property terms** — categorical Property values (`event_type`, `role`, `relationship_type`, …) are origin-namespaced `property_terms` rows; Observations store `value_term_id`. Large product/plugin term sets; researcher **Add custom…** in the picker (rename/delete when unused) — **no** Event types / Roles admin destinations. `subjects.label` stays graph-only working identity. Plugins that add bridge (or kind) subjects declare term sets + capabilities in the same registry. | §11.2, interpretation model §5.1.1, [Spike 7 S7-01b](../deployment-plan/spike-7/deployment-plan.md) |
...
- **Property terms** (`property_terms` + `value_type = term`) for kind/edge Properties (`event_type`, `role`, `relationship_type`). Settled as decision 23 / [Spike 7 **S7-01b**](../deployment-plan/spike-7/deployment-plan.md) — land before Observations write those Properties as free text. Composer uses a term picker (± Add custom); no Event types / Roles CatalogVocabulary destinations.
