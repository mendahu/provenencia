...
- **Non-translated glyphs / brand:** use `Text(verbatim:)` (e.g. `/`, `·`, `—`, `›`, `Provenencia`). Bare `Text("—")` is treated as a localizable key; with `SWIFT_EMIT_LOC_STRINGS = YES`, Xcode rewrites `Localizable.xcstrings` on build and re-adds those junk entries after you delete them.
