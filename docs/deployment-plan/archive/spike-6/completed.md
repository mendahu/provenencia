...
| [S6-05](#s6-05--dogfood-close--go-nogo) | Docs | Honesty dogfood, **Go**, composability, archive |
...
| --- | --- |
| **Kind** | Docs / verdict |
| **Depends on** | S6-01…S6-04 |
| **Deliverables** | Done. Honesty pass against the dogfood bar; **Go** verdict; GraphCanvas reuse contract documented; spike archived. |
| **Out** | SemVer bump; Citations / Observations / real connect macros; extraction refactor. |

#### Verdict: **Go**

Canvas stays the only Subject surface. Next Interpretation work can take Citations, Observations, and durable connect macros without a subject-list workaround.

#### Dogfood bar (honesty pass)

Manual pass on a real project Source after S6-04 AppKit pointer ownership:

| # | Bar item | Result |
| --- | --- | --- |
| 1 | Evidence graph opens (no stub) | **Pass** — `EvidenceGraphView` on `.sourceGraph` |
| 2 | Pinch / wheel zoom; trackpad pan; empty click-drag pan | **Pass** — AppKit document owns sequences |
| 3 | Arm Add Person/Event/Place → click → create → persists across relaunch | **Pass** |
| 4 | Drag card → snap → persist on gesture end; cold-launch click/drag | **Pass** |
| 5 | Connect A→B under zoom → bridge + edges | **Pass** |
| 6 | VoiceOver Subjects/Links rotors; Esc / Cancel; arrow-move selected | **Pass** |
| 7 | Neutral geometry module (§13) | **Pass** — see composability below |

**Provisional-link honesty (gotcha #6):** bridge cards show permanent copy (“Prototype link — not cited evidence”) in the body and VoiceOver label. Endpoints live in `EvidenceProvisionalLinkStore` (Application Support), not as cited Observations. Dogfood must not treat lines as research evidence.

#### Composability (bar item 7 / design note §13)

| Layer | Owns | Must not own |
| --- | --- | --- |
| `macos/App/Features/GraphCanvas/` | Pan/zoom, AppKit pointer, grid math, edge geometry | `CatalogSubject`, Evidence kinds, store/FFI, product rotors, card chrome |
| `macos/App/Features/EvidenceGraph/` | Palette, cards, provisional links, create dialog, a11y rotors, mode mapping | Scroll / magnification gesture ownership |

**Transfers later:** scroll shell, pointer controller + hit targets, viewport convert, optional grid/edge helpers.

**Does not transfer (§13.3–§13.4):** auto-layout, virtualization, polymorphic `subject_positions`, Evidence card chrome, Source-scoped fixed board size.

Import audit: zero Evidence / catalog types under `GraphCanvas/`. Reuse contract: [`macos/App/Features/GraphCanvas/README.md`](../../../../macos/App/Features/GraphCanvas/README.md). No extraction refactor in this step — boundaries already matched the design note.
