...
| Spike | Goal |
| --- | --- |
| [Spike 5](spike-5/) | Interpretation foundation up to the canvas: candidate refs, `subject_types` + seed, audited `subjects`, layout storage, FFI, nested Sources config (Subject types / fields stubs), Sources list → **Evidence graph** stub. **Product dogfood done** (S5-01…S5-09); **S5-10** floating-menu unify open before archive. **No Subject UI** — canvas is Spike 6. Design: [`ideas/interpretation-graph-ui.md`](../ideas/interpretation-graph-ui.md). Plan: [`spike-5/deployment-plan.md`](spike-5/deployment-plan.md). |
