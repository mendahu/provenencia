...
37. **`url` metadata is a shape check, then a browser open** (S8-07) — Set requires `URL`/`URLComponents` with `http`/`https` and a host (same rule in Go `validateValue`). No network. Click opens the default browser. Do not add a `PVLink` primitive (`PVColor.textLink` + underline). Do not accept `file:` / `javascript:` / scheme-less `example.com`.
