...
| --- | --- | --- | --- |
| S8-D5 | [`archive/S8-D5-sources-list.md`](archive/S8-D5-sources-list.md) | **S8-08** | Graph-zone subject + observation counts (uncited trailing if > 0); 248pt zone; `0 subjects · not started`; counts on their own session map, not `sourcesList`. |
...
- **Sources list** is the split-row file | graph destination. **S8-08** shipped graph-progress counts (subjects + observations + uncited) on `.sourceGraphProgress`, not on the `sourcesList` payload.
