...
- **Citation composer** is a navigable place (Option B): viewer \| form. **S8-10** shipped Layout A + the 1500pt two-column form (Citation is the document; subjects are per row). **S8-D8** changes how it saves (row-level Save / Revert / Delete, Save reading, Done + unsaved guard) and moves Connect's role choice into it. Later briefs **extend the S8-D8 frames** — do not restyle the Spike 7 subject-locked shell or draw a footer Save. Locators stay unless a brief says so.
