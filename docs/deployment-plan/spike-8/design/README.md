...
| S8-D3 | [`S8-D3-graph-visuals.md`](S8-D3-graph-visuals.md) | PR **S8-06** | Graph card visuals: conflict badge + negated badge; more items may join |
...
- **Evidence graph cards** already list every Observation as its own row. Competing values on one Property are legal. Negative polarity is a denial, not a missing line. **S8-D3** adds notice badges, not a resolve flow.
