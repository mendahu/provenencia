...
| S8-D3 | [`S8-D3-graph-visuals.md`](S8-D3-graph-visuals.md) | PR **S8-06** | Graph card visuals; first item: conflict badge on repeated Properties |
...
- **Evidence graph cards** already list every Observation as its own row. Competing values on one Property are legal. **S8-D3** adds a notice badge, not a resolve flow.
