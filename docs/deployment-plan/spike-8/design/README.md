...
| --- | --- | --- | --- |
| S8-D4 | [`S8-D4-source-page.md`](S8-D4-source-page.md) | PR **S8-07** | Restore shipped metadata + dates as text + delete + clickable `url` (jump already on the board) |
...
2. **Start with S8-D4** (Source page enhancements). Paste **one** open brief (it already contains the [working-rules](claude-design-working-rules.md) ritual + kit reach-for table). New briefs: [`add-design-brief`](../../../../.cursor/skills/add-design-brief/SKILL.md).
...
- **Source page** is filing (identity, metadata, artifacts, notes). Metadata chrome matches the **shipped** quick-add / quick-edit. Values are text (`url` is text-shaped and **clickable** in the default browser); catalog dates use that same row. Saved values can be deleted (`ClearSourceMetadata`). Jump to Evidence graph is **already on the S8-D4 board**; **S8-07** implements it. No Artifact → graph stays blocked.
